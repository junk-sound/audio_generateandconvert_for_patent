...
def main_sine(freqeuncy, amplitude, duration, file_name):
    ...
    parser.add_argument('-t', '--time', help="Duration of the wave in seconds.", default=duration, type=int)
    parser.add_argument('-a', '--amplitude', help="Amplitude of the wave on a scale of 0.0-1.0.", default=amplitude,
                        type=float)
    parser.add_argument('-f', '--frequency', help="Frequency of the wave in Hz", default=freqeuncy, type=float)
    parser.add_argument('-filename', '--filename', help="The file to generate.", default=file_name)
...
def main_square(freqeuncy, amplitude, duration, file_name):
    parser = argparse.ArgumentParser(prog="wavebender")
    parser.add_argument('-c', '--channels', help="Number of channels to produce", default=2, type=int)
    parser.add_argument('-b', '--bits', help="Number of bits in each sample", choices=(16,), default=16, type=int)
    parser.add_argument('-r', '--rate', help="Sample rate in Hz", default=44100, type=int)
    ...
    args = parser.parse_args()
    # each channel is defined by infinite functions which are added to produce a sample.
    channels = ((square_wave(args.frequency, args.rate, args.amplitude),) for i in range(args.channels))
    # convert the channel functions into waveforms
    samples = compute_samples(channels, args.rate * args.time)
    # write the samples to a file
    if args.filename == '-':
